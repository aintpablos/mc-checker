...
def logscreen():
    ...
    cmp1 = cpm
    cpm = 0
    utils.set_title(f"MSMC  | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Valid Mail: {vm}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def cuiscreen():
    ...
    utils.set_title(f"MSMC  | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Valid Mail: {vm}  -  Cpm: {cpm1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def Main():
    ...
    try:
        ...
        time.sleep(2)
        Main()
    print(Fore.LIGHTBLUE_EX+f"Proxy Type: [1] Http\s - [2] Socks4 - [3] Socks5 - [4] Tor {Fore.LIGHTBLACK_EX}{Fore.LIGHTBLUE_EX} - [5] None")
